...
public:
  LEDDemo(BLE &ble, events::EventQueue &event_queue)
      : _ble(ble), _event_queue(event_queue), _alive_led(A1, 0),
        _actuated_led(A0, 0), _led_uuid(LEDService::LED_SERVICE_UUID),
        _led_service(NULL),
...
private:
  ...
  /**
   * This callback allows the LEDService to receive updates to the ledState
   * Characteristic.
   *
  ...
  void on_data_written(const GattWriteCallbackParams *params) {
    ...
    // if ((params->handle == _led_service->getValueHandle()) && (params->len <=
    // 2)) {
    if (*(params->data) == 10 )
    {
      _actuated_led = 0;
